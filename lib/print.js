...
console.log(` ╭━━━━━━━━━━━━━━𖡼 
 ...
 ┃ ❖ ${chalk.white.bold('Action:')} ${await formaTxtStub(WAMessageStubType[m.messageStubType])} 
 ...
 ┃ ❖ ${chalk.white.bold('Type of message:')} ${chalk.bgBlueBright.bold('[%s]')} 
 ╰━━━━━━━━━━━━━━𖡼`.trim(),
    me + (conn.user.name == undefined ? '' : ' ~' + conn.user.name) + `${conn.user.jid == global.conn.user.jid ? '' : ' 【𝗬𝗕 - 𝗦𝗨𝗕 𝗕𝗢𝗧】'}`,
    sender,
    m ? '' : '',
    `🆙 ${user.level} / ❇️ ${user.exp} / 💎 ${user.limit} / 🐱 ${user.money}`,
    (m.chat.includes("@(link unavailable)") ? `${chalk.white.bold('Group:')} ${chalk.magentaBright(`${m.chat} ~${chat}`)}` : `${chalk.white.bold('Private')} ${chalk.greenBright(`${m.sender} ~${user.name ? user.name : conn.getName(m.sender)}`)} `),
filesize,
filesize === 0 ? 0 : (filesize / 1009 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1),
['', ...'KMGTP'][Math.floor(Math.log(filesize) / Math.log(1000))] || '', await formatMessageTypes(m.mtype) : 'No especificado'  //? m.mtype.replace(/message$/i, '').replace('audio', m.msg.ptt ? 'PTT' : 'audio').replace(/^./, v => v.toUpperCase()) : ''
)
