...
console.log(`
╭━━━━━━━━━━━━━━━━━━━━━━━╮
┃ 🌟 ${chalk.cyan.bold('Bot:')} ${chalk.bgMagenta(chalk.white.bold('%s'))}
┃ ⏰ ${chalk.cyan.bold('Horario:')} ${chalk.black.bgWhite.bold('%s')}
┃ 🔍 ${chalk.green.bold('Acción:')} ${await formatMessageStubType(m.messageStubType, name_user)}
┃ 👤 ${chalk.green.bold('Usuario:')} ${chalk.white('%s')} / ${chalk.bgMagentaBright.bold(user.role.replace(/\*/g, ''))}
┃ 📂 ${chalk.green.bold('Recursos:')} ${chalk.cyan('%s%s')}
┃ ✉️ ${chalk.yellow.bold('%s')}
┃ 💾 ${chalk.redBright.bold('Peso:')} ${chalk.blueBright('%s (%s %sB)')}
┃ 📁 ${chalk.redBright.bold('Tipo:')} ${chalk.greenBright.bold('[%s]')}
╰━━━━━━━━━━━━━━━━━━━━━━━╯`.trim(),
me + (conn.user.name == undefined ? '' : ' ~' + conn.user.name) + `${conn.user.jid == global.conn.user.jid ? '' : ' ║ʚɞ 𝗦𝗨𝗕 𝗕𝗢𝗧 ʚɞ║'}`,
(m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date).toTimeString(),
sender,
m ? '' : '',
`🆙 ${user.level} / ✨️ ${user.exp} / 🍪 ${user.cookies}`,
(m.chat.includes("@g.us") ? `${chalk.green('Grupo »')} ${chalk.yellowBright.italic(`${m.chat} ~${chat}`)}` : `${chalk.green('Privado »')} ${chalk.yellowBright.italic(`${m.sender} ~${user.name ? user.name : conn.getName(m.sender)}`)} `),
filesize,
