...
const { state, saveCreds } = await useMultiFileAuthState("MiniSession")
...
if (!conn.authState.creds.registered) {
  ...
  if (conn.requestPairingCode) {
    ...
    console.log(chalk.yellow(`👑 CÓDIGO DE VIMCULACIÓN 👑`, code))
  } else {
  }
}
...
if (!opts['test']) {
  if (global.db) {
    setInterval(async () => {
      ...
      if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', 'MiniJadiBot'], tmp.forEach((filename) => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])))
    }, 30 * 1000)
  }
}
