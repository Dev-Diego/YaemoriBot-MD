...
await parentw.sendMessage(global.channelid, { text: '🚩 Sub-Bot eliminado.' }, { quoted: fkontak })
} catch(err) {
if (err.code === 'ENOENT' && err.path === `./${jadi}/${uniqid}`) {
await parentw.sendMessage(global.channelid, { text: "🍟 No cuentas con ninguna sesión de Sub-Bot." }, { quoted: fkontak })
} else {
await m.react(error)
}}}
