...
//m.react('🕒') 
  if (!text) return conn.reply(m.chat, `🚩 Te faltó un link de algun video de tiktok\n🍁 Ejemplo: {usedPrefix + command} https://vm.tiktok.com/ZM6UHJYtE`, m, rcanal);
  if (!/(?:https:?\/{2})?(?:w{3}|vm|vt|t)?\.?tiktok.com\/([^\s&]+)/gi.test(text)) return conn.reply(m.chat, `🚩 Te faltó un link de algun video de tiktok\n🍁 Ejemplo: {usedPrefix + command} https://vm.tiktok.com/ZM6UHJYtE`, m, rcanal);
