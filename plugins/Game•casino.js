...
if (args.length < 1) return conn.reply(m.chat, '🚩 Ingresa la cantidad de ' + `Chocolates 🍫` + ' que deseas aportar contra' + ` *YaemoriBot-MD*` + `\n\n` + '`Ejemplo:`\n' + `> *${usedPrefix + command}* 100`, m, rcanal)
if (user.chocolates >= count * 1) {
user.chocolates -= count * 1
if (Aku > Kamu) {
conn.reply(m.chat, '`🍟 Veamos que numeros tienen!`\n\n'+ `➠ *YaemoriBot* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *PERDISTE* ${formatNumber(count)} Chocolates 🍫.`.trim(), m, rcanal)
} else if (Aku < Kamu) {
